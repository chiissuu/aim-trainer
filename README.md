...
La quinta entrada es tu mejor resultado desde que se abrió la página. Solo se sustituye cuando una partida obtiene más puntos; una puntuación igual conserva la marca y el alias que se registraron primero. La tabla se ordena de mayor a menor puntuación y, en caso de empate, queda delante quien consiguió antes esa marca. La fila del usuario se destaca con color y «(tú)». Un alias igual al de un rival no cambia su identificación como usuario.
...
- [ ] Ocho aciertos y dos fallos muestran 80% de precisión.
- [ ] Clics fuera del tablero o fuera del tiempo de partida no cuentan.
- [ ] Reiniciar y volver a jugar restablecen fallos y precisión; el resultado final conserva ambos valores en el marcador.
...
- [ ] Reiniciar durante la partida restablece los puntos, los 15 segundos y los objetivos.
- [ ] Reiniciar varias veces no acelera el contador ni duplica los objetivos.
- [ ] Al terminar aparece el formulario de alias; un nombre vacío o formado solo por espacios cambia la lista de requisitos al color de error.
...
- [ ] Se ordenan los puntos de mayor a menor y los empates conservan delante a quien consiguió antes la puntuación.
- [ ] El alias se muestra como texto, sin interpretar etiquetas HTML.
- [ ] Una partida inferior o igual no sustituye la mejor marca; una puntuación superior sí la actualiza.
...
Comprobaciones adicionales del agente para la fase 4: nueve puntuaciones (0, 36, 37, 47, 48, 53, 54, 60 y 61), todos los empates, alias vacío y de más de 20 caracteres, limpieza de espacios, HTML como texto, envío repetido, cinco filas, focos y reinicio del formulario entre partidas. También se comprobó que una puntuación inferior o igual conserva la primera mejor marca y que una superior actualiza sus puntos y alias. Estas comprobaciones utilizaron un DOM y un reloj simulados y complementan la revisión manual general del alumno.
...
- **Clasificación local:** pedí continuar con el formulario y los rivales acordados. El agente implementó validación del alias, creación de filas con `textContent`, ordenación por puntos y desempate. Después pedí presentar los requisitos del alias como una lista y cambiarla al rojo de error cuando el nombre no sea válido. Para las partidas posteriores elegí conservar en variables una única mejor marca del usuario: solo se actualiza al superar sus puntos anteriores y se pierde al recargar, sin usar `localStorage`. La revisión de estas funciones por mi parte queda pendiente.
